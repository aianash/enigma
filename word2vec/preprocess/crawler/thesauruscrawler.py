...
class ThesaurusCrawler:
	...
	def crawl_thesaurus(self):
		...
		try:
			...
			for word in word_list_seed:
				...
				json_word_context = self.get_json_word_context(word, word_context_tuple)
	...
	def get_json_word_context(self, word, word_syn_related_tuple):
		if word_syn_related_tuple is None or word is None:
			self.logger.info("Error: Invalid word context tuple")
			return

		try:
			word, syn_list, related_word_list = word_syn_related_tuple
			related_words_map = {}
